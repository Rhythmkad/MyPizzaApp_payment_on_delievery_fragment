...
//        mMyOrdersList.add(new OrderHistoryBeanClass(getString(R.string.chicken_pepper_barbecue),getString(R.string.chicken_pepper_barbecue_details), R.drawable.chicken_pepper_barbeque,R.drawable.non_veg_label, "21", "Regular | Cheese Burst | Extra Cheese",false));
    ...
    public void onClick(View view) {
        switch (view.getId()){
            case R.id.submit_order_btn:
                ((CommonActivity)mContext).callFragments(GlobalConstants.FRAGMENT.CONFIRM_ORDER_FRAGMENT, GlobalConstants.FRAGMENT.CONFIRM_ORDER_FRAGMENT, 0,false,mCompleteOrderList, mSubTotalTv.getText().toString());
                break;
        }
    }
    ...
    public void subTotal(int subTotal) {
        mSubTotalTv.setText("Subtotal $"+subTotal);
    }
