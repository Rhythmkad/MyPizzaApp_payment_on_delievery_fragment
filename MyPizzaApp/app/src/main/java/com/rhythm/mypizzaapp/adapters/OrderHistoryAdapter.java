...
public class OrderHistoryAdapter extends RecyclerView.Adapter<OrderHistoryAdapter.ViewHolder> implements View.OnClickListener  {
    ...
    public void onBindViewHolder(@NonNull ViewHolder viewHolder, int position) {
        ...
        for (OrderHistoryBeanClass i : mOrderList){
            mSubtotalAmount += Integer.parseInt(mOrderList.get(position).getPizzaPriceTotal().substring(mOrderList.get(position).getPizzaPriceTotal().indexOf(" ") + 1));
        }
        mRecyclerClickListener.subTotal(mSubtotalAmount);
